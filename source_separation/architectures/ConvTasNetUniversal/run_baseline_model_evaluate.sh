...
if python -c "import tensorflow as tf;print(tf.reduce_sum(tf.random.normal([1000, 1000])))"; then
...
python ${SCRIPT_PATH}/evaluate.py -cp=${BASELINE_MODEL_DIR}/baseline_model -mp=${BASELINE_MODEL_DIR}/baseline_inference.meta -dp=${DEV_DATA_DIR}/ssdata_reverb/eval_example_list.txt -op=${OUTPUT_DIR}
