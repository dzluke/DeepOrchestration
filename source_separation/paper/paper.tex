...
\begin{document}
	...
	The second type of model we tested are ones trained for universal sound separation (TDCN++, NMF), which \cite{tdcnpp} defines as \textit{separating mixtures of arbitrary sounds of different types}. 
			...
			\textbf{Universal Sound Separation Conv-Tasnet}\\
		...
		\begin{figure}[t]
			\centering
				...
				\caption{Diagram showing the evaluation mathod. Note that in both orchestrations with split orchestra, the split is the same.}\label{fig:full_diagram}
		\end{figure}
