...
def decode_wav(wav):
    ...
    waveform, _ = tf.audio.decode_wav(
        audio_bytes, desired_channels=1, desired_samples=-1
    )
...
def separate(input_path, output_path, ckpt, mtgph, target_sr=44100):
    ...
    # return audio resampled to 44.1kHz (resample() supports multichannel audio)
    return librosa.resample(separated_waveforms, orig_sr=16000, target_sr=target_sr)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate a source separation model.")
    parser.add_argument(
        "-cp",
        "--checkpoint_path",
    ...
    parser.add_argument(
        "-mp", "--metagraph_path", help="Path for inference metagraph.", required=True
    )
    parser.add_argument(
        "-ip", "--input_path", help="Path for input file", required=True
    )
    parser.add_argument(
        "-op", "--output_path", help="Path of resulting csv file.", required=True
    )
    args = parser.parse_args()

    separate(
        args.checkpoint_path, args.metagraph_path, args.input_path, args.output_path
    )
