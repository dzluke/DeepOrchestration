...
After training these models on varying combinations of 10 instruments, we use perform orchestration by feeding target sounds to the networks and synthesizing a solution from the most likely samples output from the system. We then compare our orchestrations to Orchidea's solutions for the same targets, through both qualitative and quantitative means. For quantitative comparison, we compute a specific distance metric on the amplitude spectrums of the target and solution, comparing the distance between Orchidea's solution to the target and our model's solution to the target.
...
This paper is an extended version of a paper by the same title in \textit{The 2020 Joint Conference on AI Music Creativity} \cite{Cella2020c}. This version includes additional figures and extended information on our baseline testing and orchestration process. The codebase for this paper can be found at \url{https://github.com/dzluke/DeepOrchestration} and you can listen to audio examples at \url{https://dzluke.github.io/DeepOrchestration/}.
...
As described in Sec. \ref{sec:baseline}, three baseline classifiers have been trained on simplified versions of our problem. They have been trained to either identifying only the instruments present, or the instruments and pitch classes present.
...
\begin{figure}
  \center{
  \includegraphics[width=0.8\textwidth]{../ISMIR_2020/figs/CNN_vs_ResNet.png}}
...
In order to have a better comparison, we did not allow Orchidea to use any of the advanced features: we did not apply any symbolic constraints or harmonic analysis and we forced it forced to use all 10 instruments in each solution. This creates a more fair comparison, since our models are unable to create constrained or sparse solutions.
