...
from parameters import TIME_LENGTH, RATE, N_FFT, MEL_HOP_LENGTH, N_MELS, rdm_granularity, instr_filter
...
def combine(soundlist):
    mixed_file = np.zeros((1, 1))
    sr = 0
    for sound in soundlist:
        sound_path = os
        sfile, sr = librosa.load(sound, sr=None)
        if len(sfile) > TIME_LENGTH*sr:
            # randomly select one part of the raw audio
            n = np.random.randint(0, len(sfile)-TIME_LENGTH*sr)
            sfile = sfile[n:n+TIME_LENGTH*sr]
...
class OrchDataSet(data.Dataset):
    '''
        Main class for generating the dataset for SampleRNN.
    '''
    ...
    def __getitem__(self, idx):
        '''
            ...
            For SampleRNN: returns a combined sound
        '''
        # stft = None
        # list_samp = []
        ...
        for i in sample_list_to_combine:
            samp = self.db[i[0]][i[1]][i[2]]
            list_samp.append(samp)
        combined_sound = combine(list_samp)
